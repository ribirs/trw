...
const updateNotes = {
   "0.8" : ["프레스티지 업데이트"
           ,"T15 무사 유화(SSR) 부터 프레스티지 진행이 가능해집니다."
           ,"프레스티지 진행시, 지금까지 진행한 모든 기록이 초기화 되며"
           ,"프레스티지 2회차로 새롭게 진행하게 됩니다."
           ,"이때, 지금까지 진행한 기록에 따라 프레스티지 포인트를 획득하게 되며"
           ,"기존 1회차보다 편하고 쾌적한 플레이가 가능해집니다."
           ,"💰 1M당 🏅 1씩 획득"
           ,"최고 스테이지 만큼 x🏅 로 획득"
           ,"+ 카드 만렙 돌파 횟수 x10🏅"
           ,"+ 카드 100,250,500,1000,2500킬 x1,2,3,4,5🏅"
           ,"+ 업그레이드 횟수 만큼 +🏅"
           ,"🏅로 카드를 직접 구매가 가능해집니다."
           ,"단, 해당 카드를 보유하고 있어야 합니다."
           ,"🏅전용 업그레이드가 추가됩니다." ],

  "0.72" : ["LR급 카드 구매 추가"
           ,"GR급 카드 구매 추가"
           ,"신규 카드구매는 업그레이드가 없습니다."
           ,"다이아로 카드구매 비용 소폭 감소"
           ,"다이아로 카드구매시 21 > 20장 획득"
           ,"신규 카드구매 해금시, 안내메시지 추가"
           ,"카드 구매시 기본 카드등급-1 감소"  ],
...
function DefaultSetting() {
  ...
  //Max 38
  version = 0.72;
  document.title = `Card Collection Game v${version}`;
  Quest = 1;
  MaxBonusAtk = 0;
  MaxBonusMhp = 0;
  MaxBonusDef = 0;
  ...
  UpgAbil2 = [
  // 프레스티지 연속 업그레이드 0~9 💰 획득량
  { Value: 20, Have: 0, Name: "💰 획득량 2배", Num: 1, Max: 10 },
  { Value: 200, Have: 0, Name: "💰 획득량 4배", Num: 2, Max: 10 },
  { Value: 800, Have: 0, Name: "💰 획득량 8배", Num: 3, Max: 10 },
  { Value: 1600, Have: 0, Name: "💰 획득량 16배", Num: 4, Max: 10 },
  { Value: 3200, Have: 0, Name: "💰 획득량 32배", Num: 5, Max: 10 },
  { Value: 6400, Have: 0, Name: "💰 획득량 64배", Num: 6, Max: 10 },
  { Value: 12800, Have: 0, Name: "💰 획득량 128배", Num: 7, Max: 10 },
  { Value: 25600, Have: 0, Name: "💰 획득량 256배", Num: 8, Max: 10 },
  { Value: 51200, Have: 0, Name: "💰 획득량 512배", Num: 9, Max: 10 },
  { Value: 102400, Have: 0, Name: "💰 획득량 1024배", Num: 10, Max: 10 },
  ];
  UpgAbil3 = [
  // 프레스티지 단일 업그레이드 
  { Value: 100, Have: 0, Name: "🗡️추가 공격력 100% 증가", Num: 1, Max: 1 },
  { Value: 1000, Have: 0, Name: "❤️추가 체력 100% 증가", Num: 1, Max: 1 },
  { Value: 10000, Have: 0, Name: "🛡️추가 방어력 +25%", Num: 1, Max: 1 },

  { Value: 500, Have: 0, Name: "사망 패널티 삭제", Num: 1, Max: 1 },
  { Value: 5000, Have: 0, Name: "자동 회복 상시 적용", Num: 1, Max: 1 },
  { Value: 50000, Have: 0, Name: "강화석 획득 확률 +25% 증가", Num: 1, Max: 1 },
  
  { Value: 80, Have: 0, Name: "카드 구매시, 가장 낮은 등급 나오지 않음", Num: 1, Max: 1 },
  { Value: 800, Have: 0, Name: "💰로 (10+1)카드 구매시, +2장 더 나옴(총13장)", Num: 1, Max: 1 },
...
function UIRendering() {
  ...
  if (Diamond > 0) {
    ...
    DiaDisplay.textContent = ``;
  }
  if (PrestigePoint > 0) {
    ppDisplay.textContent = `${numk(PrestigePoint)}🏅`;
  } else {
    ppDisplay.textContent = ``;
  }
...
function NewCardMsg() {
  if (Goal === 9) {
    LogAdd(`R~SR급 카드 구매가 해금됩니다.`,'cyan');
  }
  if (Goal === 14) {
    LogAdd(`SR~SSR급 카드 구매가 해금됩니다.`,'cyan');
  }
  if (Goal === 19) {
    LogAdd(`SSR~UR급 카드 구매가 해금됩니다.`,'cyan');
  }
  if (Goal === 24) {
    LogAdd(`UR~LR급 카드 구매가 해금됩니다.`,'cyan');
  }
  if (Goal === 29) {
    LogAdd(`LR급 카드 구매가 해금됩니다.`,'cyan');
  }
  if (Goal === 34) {
    LogAdd(`GR급 카드 구매가 해금됩니다.`,'cyan');
  }

  const unlockGoals = [14, 19, 24, 29, 34];
  if (PrestigeCnt >= 0 && PrestigeCnt < unlockGoals.length) {
    if (Goal === unlockGoals[PrestigeCnt]) {
      const nextPrestige = PrestigeCnt + 2;
      LogAdd(`프레스티지 ${nextPrestige}${nextPrestige >= 6 ? '+회차' : '회차'}가 해금됩니다.`, 'gold');
    }
  }
}
function ShowElement() {
  ...
  if (Goal >= 9) {
    document.getElementById(`ShopBtn21`).classList.remove('hidden');
    document.getElementById(`ShopBtn22`).classList.remove('hidden');
    document.getElementById(`ShopBtn23`).classList.remove('hidden');
  }
  if (Goal >= 14) {
    document.getElementById(`ShopBtn31`).classList.remove('hidden');
    document.getElementById(`ShopBtn32`).classList.remove('hidden');
    document.getElementById(`ShopBtn33`).classList.remove('hidden');
    
  }
  if (Goal >= 19) {
    document.getElementById(`ShopBtn41`).classList.remove('hidden');
    document.getElementById(`ShopBtn42`).classList.remove('hidden');
    document.getElementById(`ShopBtn43`).classList.remove('hidden');
  }
  if (Goal >= 24) {
    document.getElementById(`ShopBtn51`).classList.remove('hidden');
    document.getElementById(`ShopBtn52`).classList.remove('hidden');
    document.getElementById(`ShopBtn53`).classList.remove('hidden');
  }
  if (Goal >= 29) {
    document.getElementById(`ShopBtn61`).classList.remove('hidden');
    document.getElementById(`ShopBtn62`).classList.remove('hidden');
    document.getElementById(`ShopBtn63`).classList.remove('hidden');
  }

  const unlockGoals = [14, 19, 24, 29, 34];
  if (
    (PrestigeCnt >= 0 && PrestigeCnt < 4 && Goal >= unlockGoals[PrestigeCnt]) ||
    (PrestigeCnt >= 4 && Goal >= unlockGoals[4])
  ) {
  ...
  if (option[0].opt === true) {
    if (Goal >= 9) {
      ...
      document.getElementById(`ShopBtn12`).classList.remove('hidden');
      document.getElementById(`ShopBtn13`).classList.remove('hidden');
    }
    if (Goal >= 14) {
      document.getElementById(`ShopBtn21`).classList.remove('hidden');
      document.getElementById(`ShopBtn22`).classList.remove('hidden');
    ...
    if (Goal >= 9) {
      ...
      document.getElementById(`ShopBtn12`).classList.add('hidden');
      document.getElementById(`ShopBtn13`).classList.add('hidden');
    }
    if (Goal >= 14) {
      document.getElementById(`ShopBtn21`).classList.add('hidden');
      document.getElementById(`ShopBtn22`).classList.add('hidden');
      document.getElementById(`ShopBtn23`).classList.add('hidden');
    }
    if (Goal >= 19) {
      document.getElementById(`ShopBtn31`).classList.add('hidden');
      document.getElementById(`ShopBtn32`).classList.add('hidden');
      document.getElementById(`ShopBtn33`).classList.add('hidden');
    }
    if (Goal >= 24) {
      document.getElementById(`ShopBtn41`).classList.add('hidden');
      document.getElementById(`ShopBtn42`).classList.add('hidden');
      document.getElementById(`ShopBtn43`).classList.add('hidden');
    }
  }
  ...
  if (UpgAbil[3].Have === 1) {
    document.getElementById("ShopBtn11").innerHTML = "N~R급 카드 랜덤 구매<br>700💰";
    document.getElementById("ShopBtn12").innerHTML = "N~R급 카드 (10+1)<br>7k💰";
  } else if (UpgAbil[2].Have === 1) {
    document.getElementById("ShopBtn11").innerHTML = "N~R급 카드 랜덤 구매<br>900💰";
    document.getElementById("ShopBtn12").innerHTML = "N~R급 카드 (10+1)<br>9k💰";
  } else {
    document.getElementById("ShopBtn11").innerHTML = "N~R급 카드 랜덤 구매<br>1k💰";
    document.getElementById("ShopBtn12").innerHTML = "N~R급 카드 (10+1)<br>10k💰";
  }
  if (UpgAbil[7].Have === 1) {
    document.getElementById("ShopBtn21").innerHTML = "R~SR급 카드 랜덤 구매<br>7k💰";
    document.getElementById("ShopBtn22").innerHTML = "R~SR급 카드 (10+1)<br>70k💰";
  } else if (UpgAbil[6].Have === 1) {
    document.getElementById("ShopBtn21").innerHTML = "R~SR급 카드 랜덤 구매<br>9k💰";
    document.getElementById("ShopBtn22").innerHTML = "R~SR급 카드 (10+1)<br>90k💰";
  } else {
    document.getElementById("ShopBtn21").innerHTML = "R~SR급 카드 랜덤 구매<br>10k💰";
    document.getElementById("ShopBtn22").innerHTML = "R~SR급 카드 (10+1)<br>100k💰";
  }
  if (UpgAbil[22].Have === 1) {
    document.getElementById("ShopBtn31").innerHTML = "SR~SSR급 카드 랜덤 구매<br>70k💰";
    document.getElementById("ShopBtn32").innerHTML = "SR~SSR급 카드 (10+1)<br>700k💰";
  } else if (UpgAbil[21].Have === 1) {
    document.getElementById("ShopBtn31").innerHTML = "SR~SSR급 카드 랜덤 구매<br>90k💰";
    document.getElementById("ShopBtn32").innerHTML = "SR~SSR급 카드 (10+1)<br>900k💰";
  } else {
    document.getElementById("ShopBtn31").innerHTML = "SR~SSR급 카드 랜덤 구매<br>100k💰";
    document.getElementById("ShopBtn32").innerHTML = "SR~SSR급 카드 (10+1)<br>1M💰";
  }
  if (UpgAbil[26].Have === 1) {
    document.getElementById("ShopBtn41").innerHTML = "SSR~UR급 카드 랜덤 구매<br>7k💰";
    document.getElementById("ShopBtn42").innerHTML = "SSR~UR급 카드 (10+1)<br>7M💰";
  } else if (UpgAbil[25].Have === 1) {
    document.getElementById("ShopBtn41").innerHTML = "SSR~UR급 카드 랜덤 구매<br>9k💰";
    document.getElementById("ShopBtn42").innerHTML = "SSR~UR급 카드 (10+1)<br>9M💰";
  } else {
    document.getElementById("ShopBtn41").innerHTML = "SSR~UR급 카드 랜덤 구매<br>1M💰";
    document.getElementById("ShopBtn42").innerHTML = "SSR~UR급 카드 (10+1)<br>10M💰";
  }
  if (UpgAbil[38].Have === 1) {
    document.getElementById("ShopBtn51").innerHTML = "UR~LR급 카드 랜덤 구매<br>7M💰";
    document.getElementById("ShopBtn52").innerHTML = "UR~LR급 카드 (10+1)<br>70M💰";
  } else if (UpgAbil[37].Have === 1) {
    document.getElementById("ShopBtn51").innerHTML = "UR~LR급 카드 랜덤 구매<br>9M💰";
    document.getElementById("ShopBtn52").innerHTML = "UR~LR급 카드 (10+1)<br>90M💰";
  } else {
    document.getElementById("ShopBtn51").innerHTML = "UR~LR급 카드 랜덤 구매<br>10M💰";
    document.getElementById("ShopBtn52").innerHTML = "UR~LR급 카드 (10+1)<br>100M💰";
  }
...
function GuideSetting() {
  ...
  switch (baseT) {
    case 1:
      ...
      break;
    case 2:
      CardMin = 5 + (UpgAbil[4].Have || 0) + (UpgAbil[5].Have || 0);
      CardMax = CardMin + 4;
      break;
    case 3:
      CardMin = 10 + (UpgAbil[19].Have || 0) + (UpgAbil[20].Have || 0);
      CardMax = CardMin + 4;
      break;
    case 4:
      CardMin = 15 + (UpgAbil[23].Have || 0) + (UpgAbil[24].Have || 0);
      CardMax = CardMin + 4;
      break;
    case 5:
      CardMin = 20 + (UpgAbil[35].Have || 0) + (UpgAbil[36].Have || 0);
      CardMax = CardMin + 4;
      break;
    case 6:
      CardMin = 25;
...
function renderCardProbGuide() {
  ...
  TList.forEach(T => {
    ...
    probs.forEach(({ index, name, percent }, i) => {
      const p = document.createElement('p');
      p.innerHTML = `T${index}-<strong>${name}</strong>:${percent}%`;
  ...
  for (let i = 0; i < SlotMax; i++) {
    ...
    if (saveStr) {
      try {
        ...
        slotDiv.innerHTML = `
          <strong>[슬롯 ${i}]</strong> v${save.version}<br>
          ${formattedDate}<br>
          프레스티지 ${save.PrestigeCnt+1}회차 
          ${numk(save.PrestigePoint || 0)}🏅<br>
          ${numk(save.gold || 0)}💰 ${numk(save.Diamond || 0)}💎<br>
          퀘스트 ${save.Quest} 진행중<br>
          Tier ${save.Goal+1 || 1} - ${CardPack[save.Goal+1 || 1].name} 전투중<br><br>
          플레이어 카드 : T${save.playerCards[0].Cod} ${save.playerCards[0].name}<br>
          카드 Lv${save.playerCards[0].Lv} / 
          ${save.playerCards[0].Xp}강화 <br>
          🗡️ ${numk(save.playerCards[0].atk)} ❤️ ${numk(save.playerCards[0].Mhp)} 🛡️ ${save.playerCards[0].def}%
        `;
      } catch (e) {
...
function loadgameBySlot(slotIndex) {
  ...
  if (saveData.PrestigeCnt !== undefined) PrestigeCnt = saveData.PrestigeCnt;
  if (saveData.PrestigePoint !== undefined) PrestigePoint = saveData.PrestigePoint;
  if (saveData.UpgradeCnt !== undefined) UpgradeCnt = saveData.UpgradeCnt;
...
function Prestige() {
  ...
  `프레스티지를 진행합니다.<br>
  기본 획득량 : ${PointCal()}🏅<br>
  킬 카운트 보너스 : ${CardKillCount()}🏅<br>
  만렙 달성 보너스 : ${CardMaxTotal()}🏅<br>
  업그레이드 보너스 : ${UpgradeCnt}🏅<br>
  <br>
  프레스티지 ${PrestigeCnt+1}회차 최대 획득량 제한 : ${PpMax()}🏅<br>
  총합 포인트 : ${GetPp()}🏅<br>
  `;
  document.getElementById("Prestige-confirm-overlay").style.display = "flex";
}


function confirmPrestige(confirm) {
  document.getElementById("Prestige-confirm-overlay").style.display = "none";
  let Pc = PrestigeCnt+1;
  let Pp = PrestigePoint + GetPp();
  if (confirm ) {
    ResetGame(1);
    PrestigeCnt = Pc;
    PrestigePoint = Pp;
    LogAdd(`프레스티지를 진행합니다.`, 'lime');
  } else {
    LogAdd(`프레스티지를 취소합니다.`, 'gray');
  }
  UIRendering();
  saveGame();
}
function PpMax() {
  return Math.min(100 * 10 ** (PrestigeCnt), 10000000);
}
function GetPp() {
  let Max = Math.min(100 * 10 ** (PrestigeCnt), 10000000);
  const total = PointCal() + CardKillCount() + CardMaxTotal() + UpgradeCnt;
  return Math.min(total, Max);
}


function PointCal() {
  return (Math.floor(gold / 1000000) * (Goal+1));
}
function CardKillCount() {
  let totalScore = 0;
  for (let i = 0; i < CardKill.length; i++) {
      const count = CardKill[i];
      let bonus = 0;
      if (count >= 2500) {
          bonus = 15;
      } else if (count >= 1000) {
...
function CardMaxTotal() {
  let count = 0;
  for (let i = 0; i < CardMax.length; i++) {
      if (CardMax[i] === 1) count++;
  }
  return count * 10;
}
